...
**Amended on:** 2026-08-01 — explicit user feedback restored a compact Library Layers tab: people need visual layer identification and a direct selected-arrow inspector, not an AI-only edit path.
...
- **Library Layers tab — restored.** In Library Focus, the v2 layer tree is a human-facing list: compact per-layer previews make a colored/directional arrow easy to find, and selecting one exposes its own editable color, thickness, end, stem, and double-ended controls. The bottom toolbar remains the default for the *next* arrow, not a hidden way to edit a placed one.
...
**Alt 3: Layers panel as long-tail follow-up.** Revisited after direct user feedback. The Library DetailRail now carries a compact Layers tab (separate from the standalone editor activity bar): previews make individual annotations discoverable, and a selected-arrow inspector offers the human edit path. Reorder/hide remain there; broader per-kind inspectors can follow real demand.
...
- [ ] **R5 (carries from origin, modified):** Standalone editor activity bar surfaces Info / Chat / Tool Config + Help; the Library DetailRail adds a Layers tab with per-layer previews and selected-arrow properties. Hover/pin/safe-triangle works.
...
- [ ] **NEW — Library Layers tab:** visual previews identify layers at a glance; a selected arrow is directly editable in the right rail without changing next-tool defaults.
