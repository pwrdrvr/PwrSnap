...
5. **GitHub `windows-signing` Environment**.
   ...
   - Permit `v*` tags and `releases/*` branches. Tags cover normal releases;
     the branch rule permits reviewed manual-dispatch retries that build an
     existing immutable tag from the fixed maintenance workflow.
...
Both the `apple-signing` and `windows-signing` Environments must permit `v*`
tags and `releases/*` branches. A tag-triggered release deploys from its tag. A
manual retry must dispatch the fixed workflow from the maintenance branch while
passing the existing tag as input, so GitHub evaluates both environment
