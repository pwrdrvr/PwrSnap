...
describe("FILTER_CHANGED", () => {
  ...
  test("project mode preserves project mode regardless of visibleIds", () => {
    // A project view IS a filter: visibleIds = the project's scene
    // captures. Stay in project mode even if the current selection
    // happens to drop out (e.g. the user starts adding captures and
    // the visible set widens to the whole library).
    const next = libraryReducer(
      { kind: "project", projectId: "sz_1", selectedRecordId: "abc", adding: false },
      { type: "FILTER_CHANGED", visibleIds: [] }
    );
...
describe("OPEN_PROJECT", () => {
  test("from grid → project mode, no selection, not adding", () => {
    const next = libraryReducer(
      { kind: "grid", selectedRecordId: null },
      { type: "OPEN_PROJECT", projectId: "sz_demo" }
    );
    expect(next).toEqual({
      kind: "project",
      projectId: "sz_demo",
  ...
  test("from focus → project mode (focus state is dropped)", () => {
    const next = libraryReducer(
      { kind: "focus", selectedRecordId: "abc", returnAnchor: ANCHOR_A },
      { type: "OPEN_PROJECT", projectId: "sz_demo" }
    );
    expect(next.kind).toBe("project");
    if (next.kind === "project") expect(next.projectId).toBe("sz_demo");
  });

  test("from project → switches to the new project id", () => {
    const next = libraryReducer(
      { kind: "project", projectId: "sz_a", selectedRecordId: null, adding: true },
      { type: "OPEN_PROJECT", projectId: "sz_b" }
    );
    expect(next.kind).toBe("project");
    if (next.kind === "project") {
      expect(next.projectId).toBe("sz_b");
      // Adding flag resets — switching projects is a fresh context.
...
describe("CLOSE_PROJECT", () => {
  test("from project → grid", () => {
    const next = libraryReducer(
      { kind: "project", projectId: "sz_demo", selectedRecordId: null, adding: false },
      { type: "CLOSE_PROJECT" }
    );
    expect(next).toEqual({ kind: "grid", selectedRecordId: null });
  });

  test("from grid → no-op", () => {
    const start: LibraryView = { kind: "grid", selectedRecordId: "abc" };
    const next = libraryReducer(start, { type: "CLOSE_PROJECT" });
    expect(next).toBe(start);
  });
});

describe("TOGGLE_ADDING", () => {
  test("flips the adding flag in project mode", () => {
    const next = libraryReducer(
      { kind: "project", projectId: "sz_demo", selectedRecordId: null, adding: false },
      { type: "TOGGLE_ADDING" }
    );
    expect(next.kind).toBe("project");
    if (next.kind === "project") expect(next.adding).toBe(true);

    const next2 = libraryReducer(next, { type: "TOGGLE_ADDING" });
    if (next2.kind === "project") expect(next2.adding).toBe(false);
  });

  test("outside project mode → no-op", () => {
    const start: LibraryView = { kind: "grid", selectedRecordId: null };
    const next = libraryReducer(start, { type: "TOGGLE_ADDING" });
    expect(next).toBe(start);
  });
});
