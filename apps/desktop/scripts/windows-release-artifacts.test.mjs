...
// carries (checked against v1.1.0-beta.3's).
...
describe("Windows release aliases", () => {
  ...
  test("uses the spelling GitHub Releases stores, with no space and no version", () => {
    ...
    // The replaced alias named an architecture no download button mentions.
    // Re-adding it would give the platform two "canonical" stable URLs again.
    expect(Object.values(WINDOWS_ALIAS_NAMES)).not.toContain("PwrSnap-windows-x64-setup.exe");
  });
