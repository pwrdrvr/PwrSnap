...
const SIZZLE_CHAT_BASE_INSTRUCTIONS = [
  ...
  "  narrated scene. Use ONE sequence scene with many beats, then keep the",
  "  narration fluid.",
  "- Beat timing: default every beat to `auto` — auto beats space",
  "  themselves evenly between the anchored beats around them, so you only",
  "  need to get the ORDER right. Anchor a beat with a `phrase` (e.g.",
  "  `Settings`) ONLY when the narration names that exact moment; reserve",
  "  explicit second `offset`s for rare precise timing.",
  "- Phrase anchors come from the TTS transcript, not directly from the",
  "  written script. When `project_get` returns a sequence scene with",
  "  `transcriptPhrases.status: \"cached\"`, choose phrase anchors from",
  "  that `phrases` list. Pick one unique start phrase per intended start",
  "  word; longer phrases that begin on the same word start at the same",
  "  time and are not meaningfully different anchors.",
  "- If `transcriptPhrases` is unavailable, or if you change the sequence",
  "  narration/script, existing phrase anchors may no longer match the",
  "  regenerated TTS transcript exactly. In that case prefer `auto` until",
  "  a fresh preview/render produces transcript phrases, then adjust phrase",
  "  anchors from the latest transcript.",
  "- Sequence timing rule: beats are start anchors into one continuous",
  "  narration track. The first beat starts at 0. A non-final beat must",
  "  use automatic end timing and continue until the next beat's anchor.",
  "  Do not set fixed `endSec` / `durationSec` except on the final beat.",
