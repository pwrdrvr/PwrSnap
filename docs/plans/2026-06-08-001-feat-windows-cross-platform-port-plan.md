...
- [x] **Installed-artifact runtime readiness smoke** - unsigned preview CI and the protected signed-release job silently install their exact NSIS artifact, launch the installed `PwrSnap.exe` with isolated userData/data/profile roots, and require migrated better-sqlite3 queries, a real Sharp/libvips PNG round trip, visible React Library readiness, renderer-to-preload-to-IPC `library:list`, and clean `app.quit()`/uninstall before artifact upload. Failure output is capped and retained from the isolated tree only.
