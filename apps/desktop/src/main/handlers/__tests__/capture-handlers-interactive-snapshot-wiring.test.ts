...
vi.mock("../../render/file-alias", () => ({
  prepareRenderedFileAlias: vi.fn()
}));
...
describe("capture:interactive snapshot production wiring", () => {
  test("Windows reveals/selects before storage, then gates before crop and persistence", async () => {
    ...
    try {
      ...
      resolvePick({
        ...
        displayId: 9,
        committedCropPath: "/tmp/pwrsnap-selector-crop/crop.png",
        previousAppPid: null,
        previousAppOrigin: "unknown"
      });
  ...
  test("a blocked Windows storage gate releases the transferred snapshot and hides once", async () => {
    ...
    try {
      ...
      expect(mocks.rm).toHaveBeenCalledWith("/tmp/pwrsnap-selector-crop", {
        recursive: true,
        force: true
      });
      expect(mocks.persistCaptureFromTempV2).not.toHaveBeenCalled();
  ...
  test("persists a renderer-transferred committed crop without consulting full-frame state", async () => {
    mocks.pickRegion.mockResolvedValue({
      ok: true,
      rect: { x: 10, y: 20, w: 300, h: 200 },
      displayId: 9,
    ...
    expect(mocks.persistCaptureFromTempV2).toHaveBeenCalledWith(
      expect.objectContaining({
        tempPath: "/tmp/pwrsnap-selector-crop/crop.png"
      })
    );
    expect(mocks.releaseSnapshot).not.toHaveBeenCalled();
    expect(mocks.rm).toHaveBeenCalledWith("/tmp/pwrsnap-selector-crop", {
  ...
  test("cleans a committed renderer crop when persistence fails", async () => {
    mocks.pickRegion.mockResolvedValue({
      ok: true,
      rect: { x: 10, y: 20, w: 300, h: 200 },
      displayId: 9,
    ...
    mocks.persistCaptureFromTempV2.mockRejectedValueOnce(new Error("persist failed"));
