...
version's three files, change `PackageVersion` in all three, and change
`InstallerUrl`, `InstallerSha256`, `ReleaseDate`, and `ReleaseNotesUrl` in the
installer and locale manifests — five values in total. Then walk the checklist
above. Stable releases are infrequent enough that the automation's setup cost
...
**Not this task.** No CI wiring is being added here — this section is the
proposal, and picking between the two is the operator's call.
