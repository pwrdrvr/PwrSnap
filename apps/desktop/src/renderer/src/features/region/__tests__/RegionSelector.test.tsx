...
type ModeInput = Omit<ModePayload, "invocationId"> & { invocationId?: number };
...
type SnapshotInput = Omit<SnapshotPayload, "invocationId"> & { invocationId?: number };
...
async function emitMode(p: ModeInput): Promise<void> {
  await act(async () => {
    const { screenUrl, ...rest } = p;
    modeHandler?.({
      ...rest,
      invocationId: p.invocationId ?? DEFAULT_INVOCATION_ID,
      captureSource:
        p.captureSource ??
...
async function emitSnapshot(p: SnapshotInput): Promise<void> {
  await act(async () => {
    snapshotHandler?.({ ...p, invocationId: p.invocationId ?? DEFAULT_INVOCATION_ID });
  });
}
...
const WINDOW_LIST_WIN_B: WindowSnapEntry = {
  ...WIN,
  windowId: 4343,
...
describe("U5 — window-picker loading and invocation correctness", () => {
  test("a duplicate list delivery cannot rewind a newer pointer highlight", async () => {
    ...
    expect(submitRegion).toHaveBeenCalledWith(
      expect.objectContaining({
        ok: true,
        invocationId: 4,
        snappedWindowId: WINDOW_LIST_WIN_B.windowId,
        fullWindow: true
      })
    );
  });
  ...
  test("a press during loading cannot retain a display target when the list resolves before mouseup", async () => {
    ...
    // A complete click after readiness picks the current HWND; Enter
    // commits it using the normal pure-window backing-buffer route.
    await mouseDown(400, 300);
    await mouseUp(400, 300);
    await keyDown("Enter");
    expect(submitRegion).toHaveBeenCalledWith(
...
async function mountScene(p: ModeInput = { mode: "auto" }): Promise<void> {
  await mount();
  await emitMode(p);
  await emitSnapshot({
