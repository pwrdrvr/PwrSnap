 ...
 *
 * Every PwrSnap window is our own Electron renderer. Even the tray
 * popover (which sits on a `vibrancy: "popover"` material) paints
 * its own backdrop over the material — the renderer owns the visible
 * surface, so the user's theme pin applies everywhere. Surface
 * backdrops in CSS use `color-mix` over the themed tokens so the
 * material translucency stays consistent in both palettes.
 */
