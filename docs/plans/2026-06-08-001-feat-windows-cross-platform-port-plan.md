...
- [x] **Installed-artifact runtime readiness smoke** - unsigned preview CI and a credential-free signed-release job silently install their exact NSIS artifact, launch installed `PwrSnap.exe` with isolated roots, acquire the production first-instance lock, install the packaged tray icon/prewarm its renderer, and require migrated better-sqlite3 queries, a real Sharp/libvips PNG round trip, visible React Library readiness, renderer-to-preload-to-IPC `library:list`, direct installed window-list JSON enumeration, and (for release) FFmpeg capability checks plus an exact RGBA/PNG encode/decode chain before clean `app.quit()`/uninstall. Global hotkeys, launch-at-login mutation, startup Codex probing, local-agent lifecycle, and updater/network activity remain explicitly suppressed. Helper paths and SHA-256 values must match installed resources; signed lanes verify sidecar Authenticode, while preview explicitly reports FFmpeg absent. Tagged publication depends on this job independently from updater smoke. Failure output is capped and retained from the isolated tree only.
...
- [ ] **First signed Windows release/updater smoke** - publish a prerelease tag, install the signed NSIS artifact on a clean Windows VM, verify `.pwrsnap` double-click, still capture, clipboard copy, and one update from prerelease to prerelease. Installed launch, renderer, SQLite, and Sharp/libvips readiness are now deterministic CI gates; updater feed/blockmap/signature selection and `quitAndInstall` remain here because the hermetic runtime smoke deliberately disables networked updater activity.
