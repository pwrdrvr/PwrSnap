// Off-thread login-shell env resolution for the cached hydration path
// (see ../shell-env-hydration.ts).
//
...
// compositing for EVERY window, so the background cache refresh runs it
// here instead. The worker gets a copy of the parent's process.env at
// construction, which is exactly the base env the resolver wants.
//
