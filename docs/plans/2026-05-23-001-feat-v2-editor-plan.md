...
**Amended on:** 2026-08-02 — explicit user feedback restored a compact Library Layers tab with inline expandable arrow inspectors: people need visual layer identification and a direct human edit path, not an AI-only one.
...
- **Library Layers tab — restored.** In Library Focus, the v2 layer tree is a human-facing list: compact per-layer previews make a colored/directional arrow easy to find, and each Arrow row has an inline, independently expandable inspector for its own color, thickness, end, stem, and double-ended controls. Selecting an arrow opens its own inspector without collapsing nearby open rows, so styles can be compared. The bottom toolbar remains the default for the *next* arrow, not a hidden way to edit a placed one.
...
**Alt 3: Layers panel as long-tail follow-up.** Revisited after direct user feedback. The Library DetailRail now carries a compact Layers tab (separate from the standalone editor activity bar): previews make individual annotations discoverable, and independently expandable inline arrow inspectors offer the human edit path. Reorder/hide remain there; broader per-kind inspectors can follow real demand.
...
- [ ] **R5 (carries from origin, modified):** Standalone editor activity bar surfaces Info / Chat / Tool Config + Help; the Library DetailRail adds a Layers tab with per-layer previews and independently expandable inline Arrow properties. Hover/pin/safe-triangle works.
...
- [ ] **NEW — Library Layers tab:** visual previews identify layers at a glance; each Arrow row can expand its direct-edit controls in place, with multiple rows open for comparison, without changing next-tool defaults.
...
- **Origin document:** [docs/brainstorms/2026-05-19-v2-layer-editor-requirements.md](../brainstorms/2026-05-19-v2-layer-editor-requirements.md) — key decisions carried forward:
  ...
  4. Right-edge activity bar pattern (MODIFIED: Library DetailRail restores a compact Layers tab for direct human layer editing)
