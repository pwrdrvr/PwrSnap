...
void ActivatePid(DWORD pid) {
  ...
  // This helper is launched by the focused PwrSnap process while dismissing
  // its permission UI, so Windows normally grants the foreground handoff.
  // The APIs remain best-effort: policy may refuse them, in which case the
  // caller deliberately has no more forceful fallback.
