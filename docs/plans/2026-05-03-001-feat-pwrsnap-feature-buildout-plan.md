...
Chromium disk cache stays enabled for `pwrsnap-cache://` image responses so renderer reloads do not constantly reserialize images, but the app caps Chromium's HTTP disk cache at 128 MiB with `--disk-cache-size`. Storage accounting is exposed through `storage:snapshot` and `storage:clearChromiumCache`, and the Library status bar / Settings → Storage page read those command-bus surfaces rather than hard-coded usage.
