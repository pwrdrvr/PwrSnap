...
//     `setContextMenu`, is the PRIMARY surface. `getBounds()`,
//     `popUpContextMenu()` and the `right-click` event are all
//     `@platform darwin,win32`, and Wayland forbids a client positioning
//     its own windows — so there is nothing to anchor a popover to and no
//     icon gesture to open it with. The popover is still reachable from the
//     menu's "Show Last Capture…" row (`toggleLinuxTrayPopover`), placed
//     under the pointer on X11 and wherever the compositor chooses on
//     Wayland. Also documented in AGENTS.md "The Linux tray is a native
//     menu" and docs/linux-tray-support.md.
//
...
export function hideTrayPopoverIfVisible(): void {
  ...
  // A Linux open still waiting on the renderer's first measurement is not
  // visible yet, so the check below would skip it — and it would then show
  // itself mid-capture. Cancel it too.
  cancelPendingLinuxTrayOpen();
 ...
 * affordance.
 *
 * `native-menu` — Linux. The native `Menu` is the PRIMARY surface, attached
 * with `setContextMenu`; the popover is reachable only from its "Show Last
 * Capture…" row (`toggleLinuxTrayPopover`), never from the icon. This is not
 * a taste call; four separate pieces of Electron's Linux surface make a
 * tray-ANCHORED popover unbuildable there, and every one of them is stated
 * in electron.d.ts (checked against the pinned electron@41.10.7 — re-check
 * on a major bump):
 *
...
function wireTrayResizeChannel(): void {
  ...
  ipcMain.on(TRAY_RESIZE_CHANNEL, (event, payload: unknown) => {
    ...
    if (trayWindow.isVisible()) {
      if (traySurfaceForPlatform(shortcutPlatformFromString(process.platform)) === "native-menu") {
        if (linuxTrayPopoverAnchor !== null) {
          placeLinuxTrayPopover(trayWindow, linuxTrayPopoverAnchor);
        }
      } else if (tray !== null) {
...
export function installTray(): Tray {
  ...
  // The Linux popover is opened only from the menu row, so its resize
  // channel is wired lazily by `toggleLinuxTrayPopover` and its renderer is
  // not pre-warmed (see `LINUX_TRAY_FIRST_MEASURE_WAIT_MS`).
...
/** Identity of the Linux open currently parked for the first measurement,
 *  or `null`. Cleared to cancel it — see `toggleLinuxTrayPopover`. */
let pendingLinuxTrayOpen: object | null = null;

/** Pointer position read when the Linux popover was last opened from the
 *  menu. Reused by the resize handler so a re-measure keeps the popover where
 *  it opened. See `placeLinuxTrayPopover`. */
let linuxTrayPopoverAnchor: Electron.Point | null = null;
 ...
 *
 * `cursor` is that point, read ONCE at the click and remembered in
 * `linuxTrayPopoverAnchor` — never re-read here. A resize re-places the
 * popover while the user is interacting with it, when the pointer is inside
 * the popover rather than on the indicator; re-reading it then would re-centre
 * the popover under wherever the mouse happens to be and make it jump.
 */
function placeLinuxTrayPopover(window: BrowserWindow, cursor: Electron.Point): void {
...
function toggleLinuxTrayPopover(): void {
  ...
  // A second pick while the first is still parked waiting for the renderer
  // is a toggle too: it cancels the parked open instead of queueing another.
  if (pendingLinuxTrayOpen !== null) {
    cancelPendingLinuxTrayOpen();
    return;
  }
  // Read the anchor NOW, while the pointer is still on the indicator — not
  // after the (up to LINUX_TRAY_FIRST_MEASURE_WAIT_MS) wait, and not on a
  // later resize. See `placeLinuxTrayPopover`.
  linuxTrayPopoverAnchor = screen.getCursorScreenPoint();
  ...
  const token = {};
  pendingLinuxTrayOpen = token;
  void waitForFirstTrayMeasurement().then(() => {
    // Cancelled meanwhile — re-picked, disposed, or a capture dismissed the
    // tray (`hideTrayPopoverIfVisible`). Opening now would put the popover on
    // screen after the capture path already decided it was gone, i.e. into
    // the user's screenshot.
    if (pendingLinuxTrayOpen !== token) return;
    pendingLinuxTrayOpen = null;
    open();
  });
}

/** Drop a Linux open parked in `waitForFirstTrayMeasurement`, if any. */
function cancelPendingLinuxTrayOpen(): void {
  pendingLinuxTrayOpen = null;
}
