...
func renderBackground() -> NSBitmapImageRep {
  ...
  // Logo: "Pwr" + "Snap" — Geist Bold (brand display), falling back to system bold
  ...
  let snapSize = "Snap".size(withAttributes: [.font: logoFont])
  let logoX = (w - pwrSize.width - snapSize.width) / 2
  ...
  "Snap".draw(at: NSPoint(x: logoX + pwrSize.width, y: logoY), withAttributes: accentAttrs)

  // Subtitle
  let subtitleFont = NSFont.systemFont(ofSize: 11, weight: .medium)
  let subtitle = "screenshots / recordings"
