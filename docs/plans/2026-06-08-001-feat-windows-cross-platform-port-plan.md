...
- [x] **NSIS installer** — [#220] electron-builder `win` target + `package-win.mjs`; the `ci:windows-package` label exercises the release prepare/archive/package boundary unsigned, while `build-preview` retains the combined Mac + Windows preview path
...
- [ ] **Provision Windows signing environment** - create the protected `windows-signing` GitHub Environment and copy PwrAgent's four `WIN_AZURE_SIGN_*` variables plus three `AZURE_*` service-principal secrets. Tagged releases fail closed; see [desktop-windows-signing.md](../desktop-windows-signing.md).
- [x] **Remove temporary unsigned release mode** - release tags cannot create a GitHub Release without the signed Windows payload. Unsigned validation remains available only through local/preview builds and `ci:windows-package`, which never receive signing credentials.
