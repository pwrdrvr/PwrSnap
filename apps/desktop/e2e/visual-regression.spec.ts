...
/**
 * Pin the RASTERIZER too, for exactly the reason the scale pin exists.
 *
 * The goldens are generated by CI, which runs the whole macOS desktop
 * suite with `PWRSNAP_E2E_DISABLE_GPU: "1"` (ci.yml) — i.e. software
 * rendering. Skia's software path and a real Apple GPU do not composite
 * antialiased geometry identically: measured on an M-series laptop, the
 * same commit differs by 21 pixels across the three goldens, every one of
 * them on the crossing of the Focus close button's two SVG strokes or a
 * similar diagonal. It reads as a real regression, is stable across runs,
 * and survives `--force-color-profile=srgb` and `--disable-lcd-text`,
 * which is what makes it so convincing.
 *
 * `pnpm test:desktop-e2e` does NOT set that env var — only the CI job
 * does — so a developer running the suite locally on a machine with a
 * working GPU gets three failures that look like broken goldens. Setting
 * it in `launchVisualPwrSnap` below makes the suite self-pinning:
 * correct however it is invoked. This assertion then proves the pin
 * landed rather than trusting it, same as the scale pin.
 */
async function expectPinnedRasterizer(app: LaunchedApp): Promise<void> {
  const softwareRendering = await app.electronApp.evaluate(({ app: electronApp }) =>
    electronApp.commandLine.hasSwitch("disable-gpu")
  );
  expect(
    softwareRendering,
    "visual goldens are generated under software rendering; see expectPinnedRasterizer"
  ).toBe(true);
}
...
async function launchVisualPwrSnap(): Promise<LaunchedApp> {
  const app = await launchPwrSnap({
    env: {
      ...
      // Pin software rendering — the goldens are generated under it (CI
      // sets this for the whole macOS suite) and a real GPU composites
      // antialiased strokes a hair differently. See
      // `expectPinnedRasterizer` for the measurement and why this is set
      // here rather than left to the caller's environment.
      PWRSNAP_E2E_DISABLE_GPU: "1"
    },
  ...
  await expectPinnedRasterizer(app);
  return app;
}
