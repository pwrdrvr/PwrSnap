...
export type PackBundleArgs = {
  ...
  sourcePng: Buffer;
  /**
   * Optional low-resolution composite thumbnail (max 1024px long edge,
   * JPEG quality 80) written as `composite_thumbnail.jpg`. Generated by
   * `buildCompositeThumbnail(compositePng, dims)`; null/undefined for
   * captures that already fit within the thumbnail target (the source
   * IS thumbnail-sized — no point storing a re-encoded copy).
   *
 ...
 * `composite_thumbnail.jpg` entry. Returns `null` when the source is
 * already at-or-below the thumbnail target — for tiny captures
 * (region grabs, message thumbnails) the source bytes themselves are
 * effectively the thumbnail and re-encoding to JPEG would BLOAT the
 * bundle, not shrink it. Tests assert byte-size monotonicity.
 *
...
export async function buildCompositeThumbnail(
  compositePng: Buffer,
  dims: { width_px: number; height_px: number }
): Promise<Buffer | null> {
  const maxDim = Math.max(dims.width_px, dims.height_px);
  if (maxDim <= COMPOSITE_THUMBNAIL_MAX_DIM_PX) return null;
...
export type PackBundleV2Args = {
  ...
  /**
   * Optional low-resolution composite thumbnail. Same shape + reasoning
   * as v1: generated via `buildCompositeThumbnail`, omitted when the
   * source is already ≤ thumbnail target. Replaces the legacy
   * full-resolution `composite.png` entry that v2 used to write.
   */
  thumbnailJpg?: Buffer | null;
};
 ...
 *   composite_thumbnail.jpg    — STORE; optional (omitted for small
 *                                captures, ≤ 1024px long edge)
 *
...
export async function persistCaptureFromTemp(
  ...
  // paired_png_filename is a vestigial manifest field — the file
  // itself is no longer written next to the bundle, but the manifest
  // still records what the basename WOULD be so a future export /
  // doctor pass can reconstruct one if needed.
  ...
  const { record } = insertOrFindCapture({
    ...
    // Paired flat PNG sibling no longer written; field stays nullable
    // in the schema for back-compat with legacy bundle-flow captures
    // that DO have one on disk (until they're swept up).
    flat_png_path: null,
    bundle_modified_at: now,
