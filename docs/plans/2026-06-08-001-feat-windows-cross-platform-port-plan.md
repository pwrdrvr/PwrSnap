...
- [x] **Isolated packaged updater smoke automation** - the protected Windows signing path produces a non-publishable, marker-gated same-source prerelease pair; a fresh credential-free hosted runner serves only the newer target from `127.0.0.1` and exercises `electron-updater` download/install/relaunch with version, database, sentinel, and install-attempt continuity checks. Tag publication depends on the gate, while `ci:windows-updater-smoke` exposes the same non-publishing path to reviewed same-repository PRs. See [windows/packaged-updater-smoke.md](../windows/packaged-updater-smoke.md).
- [ ] **First protected packaged updater smoke execution** - temporarily allow the reviewed PR merge ref in `windows-signing`, approve the opt-in job, capture the signed baseline-to-target result, then remove the rule. Ordinary PR CI can validate the harness and isolation but cannot mint the required signed pair.
