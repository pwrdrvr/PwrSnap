import type { SizzleVideoFitPolicy } from "./protocol";

export type SizzleVideoFitRenderMode =
  | "trim"
  | "freeze-end"
  | "loop"
  | "ping-pong"
...
export type SizzleVideoFitDecision = {
  requested: SizzleVideoFitPolicy;
  selected: SizzleVideoFitPolicy;
  renderMode: SizzleVideoFitRenderMode;
...
export function resolveSizzleVideoFit(args: {
  policy: SizzleVideoFitPolicy;
  sourceDurationSec: number;
  targetDurationSec: number;
}): SizzleVideoFitDecision {
