...
7. **Schema gets every UNIQUE/index/cascade it was missing.** `captures.sha256 UNIQUE`, `capture_tags(capture_id, tag_id) UNIQUE`, `tag_destinations(tag_id, destination_id) UNIQUE`, `tags(label, kind) UNIQUE`, `render_cache(capture_id, render_inputs_hash, format) UNIQUE`, `uploads(capture_id, destination_id) UNIQUE` (idempotent retries), partial index for the timeline-by-app-and-date hot path. `PRAGMA foreign_keys = ON` and explicit `ON DELETE CASCADE`/`RESTRICT`. AI-suggestion lifecycle gains `rejected_at` + `superseded_by` + `ai_run_id` so analytics has a denominator. Soft-delete moves files to `<root>/.trash/` atomically; GC just `rm -rf` after 30d. (data-integrity)
...
|---|---|
| Codex App Server protocol types | Consume the published `@pwrdrvr/codex-app-server-protocol` package, pinned exactly in `apps/desktop/package.json`. Generation now happens in `github.com/pwrdrvr/codex-app-server-protocol`; PwrSnap no longer vendors generated protocol files. PwrSnap consumes via `import type { ... } from "@pwrdrvr/codex-app-server-protocol/v2"`. |
...
Cross-repo dependency policy: PwrSnap does **not** depend on `@pwragnt/shared` or any other PwrAgnt package. The two repos remain independent. The genuinely-shared concept (the Codex App Server protocol) is published as `@pwrdrvr/codex-app-server-protocol` and consumed by both PwrSnap and PwrAgent.
...
- [x] **Codex App Server protocol package.**
  - [x] Initially shipped as a local `packages/codex-app-server-protocol/` workspace package generated from Codex CLI.
  - [x] Replaced with the published `@pwrdrvr/codex-app-server-protocol` npm package so generated protocol files live in their own repo and PwrSnap/PwrAgent consume the same version.
  - [x] Remove the root `codex:generate-protocol` script; regeneration is now done in `github.com/pwrdrvr/codex-app-server-protocol`, followed by an exact dependency bump in `apps/desktop/package.json`.
...
- [x] **Persistence.**
  ...
  - [x] Boot-time GC: clean `/tmp/pwrsnap-*` older than 1h; clean `<root>/.trash` older than 30d. _Implemented as `sweepStaleTempFiles` + `sweepTrash` in `source-store.ts`, invoked from `index.ts` at boot._
- [x] **Capture pipeline.**
...
- **Phase 8a — Windows: in progress.** No longer deferred. A working Windows
  ...
  remaining work (first signed-release smoke, auto-update, capture-trigger debounce,
...
- **Soft delete vs disk** — `captures.deleted_at` set immediately + `source-store.ts` atomically moves the file to `<root>/.trash/<uuid>.png` (same-volume rename). Undelete moves it back. GC `rm -rf` `.trash` entries older than 30d. No TOCTOU race because the move is atomic.
...
- Protocol package: `@pwrdrvr/codex-app-server-protocol` — generated and published from `github.com/pwrdrvr/codex-app-server-protocol`.
