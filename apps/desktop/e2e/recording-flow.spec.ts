...
test.describe("video float-over", () => {
  ...
  test("video asset renders <video> + GIF/MP4 buttons + Discard footer action", async () => {
    ...
    try {
      ...
      // Target the label by class: the MP4 eyebrow also carries the
      // audio toggles, whose spans a bare `span` selector would match.
      const eyebrowLabel = ".psl__copy-format-eyebrow-label";
      ...
      // The seeded take has system audio only, so the MP4 row offers
      // exactly one audio toggle (on by default) and the GIF row none.
      await expect(formatGroups.nth(0).locator(".psl__copy-audio-toggle")).toHaveCount(0);
      const audioToggles = formatGroups.nth(1).locator(".psl__copy-audio-toggle");
      await expect(audioToggles).toHaveCount(1);
      await expect(audioToggles.nth(0)).toHaveAttribute("data-track", "systemAudio");
      await expect(audioToggles.nth(0)).toHaveAttribute("aria-pressed", "true");
