 ...
 * (1024) so buildCompositeThumbnail produces a JPEG genuinely
 * resized from the source rather than a same-dimensions re-encode.
 * Pass C's preferred-image-source logic (composite == source → use
 * source) is easier to verify against a real resize. (Historical
 * note: `buildCompositeThumbnail` used to return null for small
 * inputs to skip a same-dim re-encode — that's gone, the thumbnail
 * is always written now to keep Finder + Quick Look working for
 * v2 bundles.)
 */
...
function insertCaptureRow(
  ...
  //
  // 1200×800 matches the synthetic PNG generated by makeLargePng().
  // Historical note: there used to be a `buildCompositeThumbnail` size
  // skip that returned null for sources ≤ 1024px long edge, and this
  // test had to exceed that to exercise the thumbnail write path. The
  // size skip is gone (always writes a thumbnail now); the dims here
  // still match the synthetic image so the test bytes round-trip
  // cleanly.
  db.prepare(
