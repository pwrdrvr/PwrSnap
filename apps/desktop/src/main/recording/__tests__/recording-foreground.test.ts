...
describe("recording foreground restoration", () => {
  ...
  test("production restores required permission focus before the selector", async () => {
    const source = await readFile(new URL("../../index.ts", import.meta.url), "utf8");
    const restore = source.indexOf("withRecordingForegroundRestored(() =>");
    const selector = source.indexOf("const selection = await pickRegion({", restore);

    expect(restore).toBeGreaterThan(-1);
    expect(selector).toBeGreaterThan(restore);
  });
});
